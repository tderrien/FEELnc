...
sub pathProg{
	...
	if (defined $prog){
		...
		die "pathProg: '$prog' not in your PATH..." if (! @path);
        die "pathProg: Cannot't exec '$prog'...\n"  if (-x $prog);	
	}else{
...
sub runCPAT{
    ...
    # test path program
    my $pathhexamer     =   pathProg("make_hexamer_tab.py");
    my $pathlogit       =   pathProg("make_logitModel.py");
    my $pathcpat        =   pathProg("cpat.py");
